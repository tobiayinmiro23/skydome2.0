...
import { listCartItem ,cartTotal,handleDelete,increaseProductQuantity,increaseShoeSize,decreaseProductQuantity,decreaseShoeSize} from '../features/Cart/Cart'
...
const Cart = () => {
    ...
    const {cart,total}=useSelector(item=>item.cart)
    const [Cart, setCart] = useState([])
    const [productPrice, setproductPrice] = useState(0)
    let localStorageData= localStorage.getItem('skydome')
    let cartId;
   useEffect(()=>{
    loaderTimer(setloading,400)
    window.scrollTo(0,0)
    if(localStorageData.length){
        cartId= JSON.parse(localStorage.getItem('skydome')) 
        let memory=[]
        cartId?.forEach(item =>{
                        ...
                        return <div key={cid.id}>
                            <div  style={{boxShadow:'0px 1px 3px 0px rgba(0, 0, 0, 0.378)'}} className='flex item-start w-[45rem] mb-[1.3rem] mx-[4rem] bg-[#faebd788] p-[1rem]  max-[1125px]:w-[38rem] max-[1033px]:w-[36rem] max-[1033px]:mb-[2rem] max-[1033px]:ml-[2rem] max-[1033px]:mr-[1rem] max-[898px]:w-[35rem] max-[898px]:mx-[1rem] max-[898px]:mb-[1rem] max-[855px]:w-[85%] max-[855px]:m-[auto] max-[855px]:my-[2rem] max-[743px]:w-[90%] max-[535px]:w-[95%]'>
                                <div className='one'>
                                    ...
                                    <div className=" w-[fit-content] rounded-[0.3rem] px-[0.6rem] py-[0.5rem] bg-[red] cursor-pointer mt-[1rem] hover:bg-[#e41717]" >
                                        <div onClick={()=>handleDeleteCart(cid)}  className='w-[1rem]  max-[463px]:w-[0.9rem] '><img src="/productimage/delete (1).png" alt="delete button" /></div>
                                    </div>
                                </div>
